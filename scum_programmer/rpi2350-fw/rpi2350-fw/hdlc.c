...
uint16_t _hdlc_update_fcs(uint16_t fcs, uint8_t byte) {
...
hdlc_state_t hdlc_rx_byte(uint8_t byte) {
    const bool can_handle_new_frame = (_hdlc_vars.state == HDLC_STATE_IDLE ||
...
size_t hdlc_decode(uint8_t *output) {
    size_t output_pos = 0;
    if (_hdlc_vars.state != HDLC_STATE_READY) {
        return output_pos;
    }
    ...
    while (input_pos < _hdlc_vars.buffer_pos) {
        ...
        if (current_byte == HDLC_ESCAPE) {
            ...
            if (current_byte == HDLC_ESCAPE_ESCAPED) {
                output[output_pos++] = HDLC_ESCAPE;
            } else if (current_byte == HDLC_FLAG_ESCAPED) {
                output[output_pos++] = HDLC_FLAG;
            }
            escape_byte = false;
        } else {
    ...
    _hdlc_vars.state = HDLC_STATE_IDLE;
    return output_pos - 2;
}
